...
export const MsgDeleteProviderAttributes = createMsgView(["v1beta1", "v1beta2", "v1beta3", "v1"], ({ message }) => {
  return (
    <>
      <LabelValue label="Provider" value={<Link to={UrlService.address(message?.data?.owner)}>{message?.data?.owner}</Link>} />
      {/* TODO: Add link to provider page */}
      <LabelValue label="Auditor" value={<Link to={UrlService.address(message?.data?.auditor)}>{message?.data?.auditor}</Link>} />
