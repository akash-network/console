...
export const MsgCreateDeployment = createMsgView(["v1beta1", "v1beta2", "v1beta3", "v1beta4"], ({ message }) => {
  ...
  return (
    <>
      ...
      <LabelValue label="dseq" value={<Link to={UrlService.deployment(data?.id?.owner, data?.id?.dseq)}>{data?.id?.dseq}</Link>} />
      <LabelValue label="Version" value={data?.hash || data?.version} />
      {data?.depositor && <LabelValue label="Depositor" value={<Link to={UrlService.address(data?.depositor)}>{data?.depositor}</Link>} />}
