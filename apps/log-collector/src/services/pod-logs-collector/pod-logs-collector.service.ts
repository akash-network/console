...
export class PodLogsCollectorService {
  ...
  private async startKubernetesLogStream(containerName: string, logStream: PassThrough, lastTimestamp?: number | null): Promise<void> {
    ...
    const k8sAbortController = await this.k8sLogClient.log(this.podInfo.namespace, this.podInfo.podName, containerName, logStream, logStreamOptions);
    ...
    this.loggerService.info({
      ...
      namespace: this.podInfo.namespace,
      containerName,
      aborted: k8sAbortController?.signal?.aborted
    });
  }
}
