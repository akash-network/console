...
export function generateProvider(overrides: Partial<CreationAttributes<Provider>> = {}): CreationAttributes<Provider> {
  return {
    ...
    ipRegionCode: overrides.ipRegionCode || faker.location.stateAbbr(),
    ...
    uptime1d: overrides.uptime1d || faker.number.float({ min: 0, max: 1, precision: 0.01 }),
    uptime7d: overrides.uptime7d || faker.number.float({ min: 0, max: 1, precision: 0.01 }),
    uptime30d: overrides.uptime30d || faker.number.float({ min: 0, max: 1, precision: 0.01 }),
