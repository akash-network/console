...
describe(X402Service.name, () => {
  ...
  describe("getDiscovery", () => {
    it("advertises the configured accepts derived from the canonical source, including the deploy route", () => {
      const { service } = setup();

      const discovery = service.getDiscovery();

      expect(discovery.x402Version).toBe(2);
      const topUp = discovery.resources.find(resource => resource.resource === X402_TOP_UP_ROUTE);
      ...
      // The v2.0 pay-per-deploy route is advertised from the same canonical source.
      expect(discovery.resources.map(resource => resource.resource)).toContain("POST /v1/x402/deploy");
    });
