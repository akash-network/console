...
import { type FirstPurchaseBonusGrant, StripeTransactionService } from "@src/billing/services/stripe-transaction/stripe-transaction.service";
...
export class StripeWebhookService {
  ...
  async routeStripeEvent(signature: string, rawEvent: string) {
    ...
    try {
      let bonusGrant: FirstPurchaseBonusGrant | undefined;

      switch (event.type) {
        case "payment_intent.succeeded":
          bonusGrant = await this.stripeTransaction.settlePaymentIntent(event);
          break;
        case "invoice.paid":
        case "invoice.payment_succeeded":
          ...
          bonusGrant = await this.stripeTransaction.settleInvoice(event);
          break;
      ...
      // Published after the settling transaction has committed so the bonus-granted email never fires on a
      // rolled-back grant.
      if (bonusGrant) {
        await this.domainEventsService.publish(new FirstPurchaseBonusGranted(bonusGrant));
      }
    } catch (error) {
      this.logger.error({
