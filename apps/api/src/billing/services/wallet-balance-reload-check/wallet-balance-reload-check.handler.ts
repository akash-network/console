...
import { AUTO_TOP_UP_METADATA_KEY, StripeTransactionService } from "@src/billing/services/stripe-transaction/stripe-transaction.service";
  ...
  async #tryToReloadOnFixedThreshold(resources: ReloadContext): Promise<void> {
    ...
    try {
      await this.stripeTransactionService.createPaymentIntent({
        ...
        amount: reloadAmount,
        confirm: true,
        metadata: { [AUTO_TOP_UP_METADATA_KEY]: "true" },
