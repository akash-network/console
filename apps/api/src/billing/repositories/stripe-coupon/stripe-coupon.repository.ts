...
export class StripeCouponRepository extends BaseRepository<ApiPgTables["StripeCoupons"], StripeCouponInput, StripeCouponOutput> {
  ...
  async findByStripeCustomerId(stripeCustomerId: string): Promise<StripeCouponOutput[]> {
    const items = await this.cursor
      .select()
      .from(this.table)
  ...
  async findByStripeTransactionId(stripeTransactionId: string): Promise<StripeCouponOutput[]> {
    const items = await this.cursor
      .select()
      .from(this.table)
  ...
  async countCouponsByUserId(userId: string): Promise<number> {
  ...
  async countCouponsByStripeCustomerId(stripeCustomerId: string): Promise<number> {
    const result = await this.cursor
      .select({ count: this.table.id })
      .from(this.table)
      .where(this.whereAccessibleBy(eq(this.table.stripeCustomerId, stripeCustomerId)));
