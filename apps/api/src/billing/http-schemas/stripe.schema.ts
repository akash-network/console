...
const hasCardOrLink = (data: { card?: unknown; link?: unknown }) => Boolean(data.card || data.link);
const cardOrLinkError = { message: "At least one of card or link must be provided" };
...
export const PaymentMethodSchema = PaymentMethodDetailsSchema.extend({ id: z.string() }).refine(hasCardOrLink, cardOrLinkError);
