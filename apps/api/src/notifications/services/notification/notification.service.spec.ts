...
describe(NotificationService.name, () => {
  ...
  describe("createNotification", () => {
    ...
    it("does not create default channel when user has no email and surfaces the error", async () => {
      jest.useFakeTimers();
      const { service, api } = setup();
      ...
      expect((result as PromiseRejectedResult).reason.cause).toEqual(expect.objectContaining(error));

      expect(api.v1.createDefaultChannel).not.toHaveBeenCalled();
      expect(api.v1.createNotification).toHaveBeenCalledTimes(10);
    });

    it("fails after 10 attempts if notification service is not available  ", async () => {
      jest.useFakeTimers();
      const { service, api } = setup();
