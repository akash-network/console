...
export const envSchema = z
  .object({
    ...
    POSTGRES_BACKGROUND_JOBS_CONNECT_TIMEOUT: z.number({ coerce: true }).optional().default(3),
    POSTGRES_MAX_CONNECTIONS: z.number({ coerce: true }).optional().default(20),
    POSTGRES_CONNECT_TIMEOUT: z.number({ coerce: true }).optional().default(3),
    ...
    /** Separate from SECRET_TOKEN so the Auth0 tenant holds a credential that opens one endpoint and can be rotated on its own. */
