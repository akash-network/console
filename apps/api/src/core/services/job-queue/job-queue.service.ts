...
export class JobQueueService implements Disposable {
  ...
  async cancelCreatedByPattern(query: { name: string; singletonKeyPattern: string }): Promise<void> {
    const db = await this.pgBoss.getDb();
    const schema = this.coreConfig.get("POSTGRES_BACKGROUND_JOBS_SCHEMA");
    const result = (await db.executeSql(
      `
        WITH results as (
          ...
          WHERE name = $1
            AND state = 'created'
            AND singleton_key LIKE $2
          RETURNING id
        )
        SELECT id FROM results
      `,
      [query.name, query.singletonKeyPattern]
    )) as { rows: { id: string }[] };

    this.logger.info({
      event: "JOBS_CANCELLED_BY_PATTERN",
      pattern: query.singletonKeyPattern,
      jobIds: result.rows.map(r => r.id)
    });
  }
