...
/**
 * Tokenizes a shell command into argv-style tokens the way a POSIX shell splits
 * a command line, so each token maps to one `cmdN` query param consumed by the
 * provider's lease-shell endpoint (which execs the tokens as argv, with no shell
 * re-interpretation of its own).
 *
 * A naive `command.split(" ")` corrupts any argument that legitimately contains
 * whitespace — quoted strings, paths, JSON blobs, PEM bodies — and is especially
 * broken for the post-deploy secret-injection use case (e.g.
 * `sh -c "echo SECRET=v > /run/secrets/.env"`), which must survive as the three
 * tokens `["sh", "-c", "echo SECRET=v > /run/secrets/.env"]`.
 *
 * Rules: unquoted whitespace delimits tokens (space/tab/newline — matching the
 * POSIX default `IFS`, so a carriage return is preserved, not a delimiter); single quotes
 * preserve their contents literally; double quotes preserve contents but allow
 * `\` to escape `"` and `\`; a backslash outside quotes escapes the next char.
 * Quote characters are removed from the emitted tokens (shell semantics).
 */
...
export function buildShellUrl(input: Pick<ShellExecInput, "providerBaseUrl" | "dseq" | "gseq" | "oseq" | "service" | "command">): string {
  ...
  const cmdParts = tokens.length > 0 ? tokens.map((token, i) => `&cmd${i}=${encodeURIComponent(token)}`) : [`&cmd0=`];
