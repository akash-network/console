...
describe(ShellExecService.name, () => {
  describe("buildShellUrl", () => {
    ...
    it("treats newline as a token delimiter", () => {
    ...
    it("treats tab as a token delimiter", () => {
      const input = createShellExecInput({ command: "echo\thello" });
    ...
    it("collapses mixed whitespace (spaces and tabs) between tokens", () => {
      const input = createShellExecInput({ command: "echo \t hello" });
    ...
    it("splits on multiple consecutive tabs", () => {
      const input = createShellExecInput({ command: "cmd\t\targ" });

      const url = buildShellUrl(input);

      expect(url).toContain("cmd0=cmd");
      expect(url).toContain("cmd1=arg");
      expect(url).not.toContain("cmd2=");
    });

    it("keeps a double-quoted argument with spaces as a single token", () => {
      const input = createShellExecInput({ command: 'echo "hello world"' });

      const url = buildShellUrl(input);

      expect(url).toContain("cmd0=echo");
      expect(url).toContain("cmd1=hello%20world");
      expect(url).not.toContain("cmd2=");
    });

    it("keeps a single-quoted argument with spaces as a single token", () => {
      const input = createShellExecInput({ command: "echo 'a b c'" });

      const url = buildShellUrl(input);

      expect(url).toContain("cmd0=echo");
      expect(url).toContain("cmd1=a%20b%20c");
      expect(url).not.toContain("cmd2=");
    });

    it("preserves the post-deploy secret-injection command as argv (sh -c ...)", () => {
      const input = createShellExecInput({ command: 'sh -c "echo SECRET=v > /run/secrets/.env"' });
    ...
    it("treats a backslash-escaped space as part of the token", () => {
      const input = createShellExecInput({ command: "echo a\\ b" });

      const url = buildShellUrl(input);

      expect(url).toContain("cmd0=echo");
      expect(url).toContain("cmd1=a%20b");
      expect(url).not.toContain("cmd2=");
    });
  ...
  describe("buildShellUrl edge cases", () => {
    it("treats an unquoted backslash as a shell escape (removes it)", () => {
      const input = createShellExecInput({ command: "echo \\hello\\world" });

      const url = buildShellUrl(input);

      expect(url).toContain("cmd0=echo");
      expect(url).toContain("cmd1=helloworld");
    });

    it("preserves literal backslashes inside single quotes", () => {
      const input = createShellExecInput({ command: "echo '\\hello\\world'" });
