 ...
 * the sole key), imports the JWK via `node:crypto`, and checks the signature with an allow-listed set of
 * asymmetric algorithms. Throws on any failure so callers can treat the report as `unverifiable`.
 */
export function verifyJwtWithJwks(token: string, jwks: Jwks): jwt.JwtPayload {
  ...
  const jwk = (kid ? jwks.keys.find(key => key.kid === kid) : undefined) ?? jwks.keys[0];
