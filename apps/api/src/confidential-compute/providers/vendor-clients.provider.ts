...
container.register(AMD_KDS_HTTP_CLIENT, {
  useFactory: instancePerContainerCachingFactory(c => createHttpClient({ baseURL: c.resolve(CONFIDENTIAL_COMPUTE_CONFIG).AMD_KDS_BASE_URL, adapter: "http" }))
});

container.register(NVIDIA_NRAS_HTTP_CLIENT, {
  useFactory: instancePerContainerCachingFactory(c =>
    createHttpClient({ baseURL: c.resolve(CONFIDENTIAL_COMPUTE_CONFIG).NVIDIA_NRAS_BASE_URL, adapter: "http" })
  )
});

container.register(INTEL_ITA_HTTP_CLIENT, {
  useFactory: instancePerContainerCachingFactory(c => createHttpClient({ baseURL: c.resolve(CONFIDENTIAL_COMPUTE_CONFIG).INTEL_ITA_BASE_URL, adapter: "http" }))
});
