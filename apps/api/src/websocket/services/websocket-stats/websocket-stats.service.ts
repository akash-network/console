...
export class ClientWebSocketStats {
  ...
  private user?: { currentUser: UserOutput; ability: MongoAbility };
  ...
  setUserIfExists(currentUser: UserOutput | undefined, ability: MongoAbility) {
    if (currentUser) {
      this.user = { currentUser, ability };
    }
  }
