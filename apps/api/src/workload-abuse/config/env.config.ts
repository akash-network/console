...
export const envSchema = z.object({
  ...
  WORKLOAD_ABUSE_BEHAVIOURAL_ENFORCEMENT_ENABLED: z
    .enum(["true", "false"])
    .default("false")
    .transform(value => value === "true"),
  WORKLOAD_ABUSE_BEHAVIOURAL_AGREEMENT_PROBES: z.number({ coerce: true }).int().positive().default(3),
  /** Back-to-back probes can land minutes apart, so a streak also has to span real time before it counts. */
  WORKLOAD_ABUSE_BEHAVIOURAL_MIN_WINDOW_MINUTES: z.number({ coerce: true }).int().positive().default(120)
});
