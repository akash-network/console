...
describe(WorkloadAbuseInstrumentationService.name, () => {
  it("creates a counter per outcome it reports", () => {
    ...
    expect(metricsService.createCounter).toHaveBeenCalledWith(expect.anything(), "workload_abuse_blocked_domain_lookup_failures_total", expect.anything());
  ...
  describe("recordBlockedDomainLookupFailure", () => {
    it("counts a lookup that failed, because enforcement is off while it climbs", () => {
      const { service, blockedDomainLookupFailures } = setup();

      service.recordBlockedDomainLookupFailure();

      expect(blockedDomainLookupFailures.add).toHaveBeenCalledWith(1);
    });
  });
  ...
  function setup() {
    ...
    const blockedDomainLookupFailures = mock<Counter>();
    ...
    metricsService.createCounter
      ...
      .mockReturnValueOnce(blockedDomainLookupFailures)
      .mockReturnValueOnce(domainBlocks);

    const service = new WorkloadAbuseInstrumentationService(metricsService);

    return { service, metricsService, probes, detections, enforcements, blockedDomainLookupFailures, domainBlocks };
  }
});
