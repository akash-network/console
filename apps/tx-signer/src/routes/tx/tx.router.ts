...
import { TxManagerController } from "@src/controllers/tx-manager/tx-manager.controller";
import {
...
txRouter.openapi(fundingRoute, async c => {
  const payload = await container.resolve(TxManagerController).signWithFundingWallet(c.req.valid("json"));
...
txRouter.openapi(derivedRoute, async c => {
  const payload = await container.resolve(TxManagerController).signWithDerivedWallet(c.req.valid("json"));
  return c.json(payload, 200);
});
