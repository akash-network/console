...
/**
 * To upgrade the Leap extension, you need to:
 * 1. Download the new version of the Leap extension from the Chrome Web Store (https://chromewebstore.google.com/detail/leap-wallet/fcfcfllfndlomdhbehjjcoimbgofdncg). Use https://chromewebstore.google.com/detail/download-chrome-extension/idieikhccepnagohoobmbpocdbogegij
 * 2. Rename zip file to `Leap-<version>.zip` and save it to `apps/deploy-web/tests/ui/chrome-extensions`
 * 3. Change the `LEAP_VERSION` variable here
 * 4. Remove the old Leap extension zip file
 * 5. Run `npm run test:e2e -- --headed` to verify the new extension is working
 */
...
export default async () => {
  ...
  console.log(`✅ Leap v${LEAP_VERSION} chrome extension was successfully unarchived`);
