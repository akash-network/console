...
describe(useIsOnboarded.name, () => {
  it("returns true when the managed wallet has at least one lease", () => {
    const { result } = setup({ hasWallet: true, address: "akash1abc", hasLease: true });

    expect(result.current).toBe(true);
  });

  it("returns false when the managed wallet has no lease and onboarding was not skipped", () => {
    const { result } = setup({ hasWallet: true, address: "akash1abc", hasLease: false });

    expect(result.current).toBe(false);
  });

  it("returns false when the wallet has no address yet", () => {
    const { result } = setup({ hasWallet: true, address: "", hasLease: true });

    expect(result.current).toBe(false);
  });

  it("returns false when there is no managed wallet and onboarding was not skipped", () => {
    const { result } = setup({ hasWallet: false });

    expect(result.current).toBe(false);
  });

  it("returns true when onboarding was explicitly skipped even without a lease", () => {
    const { result } = setup({ hasWallet: false, onboardingSkippedAt: "2026-07-30T00:00:00Z" });

    expect(result.current).toBe(true);
  });

  function setup(input?: { hasWallet?: boolean; address?: string; hasLease?: boolean; onboardingSkippedAt?: string | null }) {
