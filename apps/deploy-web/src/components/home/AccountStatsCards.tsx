...
export const AccountStatsCards: React.FC<Props> = ({ walletBalance, activeDeploymentsCount, costPerMonth }) => {
  return (
    <div className="grid gap-4 md:grid-cols-3">
      ...
      <Card>
        <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
          <h3 className="text-sm font-medium text-muted-foreground">Total Cost per Month</h3>
          <StatsUpSquare className="h-4 w-4 text-muted-foreground" />
        </CardHeader>
        <CardContent>
          <div className="text-2xl font-bold">
            <FormattedNumber value={costPerMonth || 0} style="currency" currency="USD" />
          </div>
        </CardContent>
      </Card>
    </div>
