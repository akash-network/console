...
export const ManagedEscrowFaqModal: React.FunctionComponent<Props> = ({ onClose }) => {
  const { minDeposit } = useChainParam();
  const { walletBalance } = useTotalWalletBalance();
  return (
    <Popup fullWidth open variant="custom" title="FAQ - Deployments" actions={[]} onClose={onClose} maxWidth="sm" enableCloseOnBackdropClick>
      {walletBalance && (
      ...
      <div className="space-y-2">
        <Alert className="space-y-2">
          ...
          <p className="text-sm text-muted-foreground">
            This deposit acts as collateral and is used to pay for the resources consumed by your application. Akash charges by the minute and once you close
            your deployment, the remaining balance is transferred back to your available balance.
          </p>
