 ...
 * hostnames map to the `accept` array and apply only when the port isn't internal
 * (mirroring the SDL, where `accept` is meaningful for externally reachable HTTP
 * traffic).
 *
        ...
        <Field className="flex-1 gap-2">
          ...
          <FieldContent>
            <ToggleGroup
              ...
              onValueChange={value => value && proto.field.onChange(value)}
...
const HttpOptionsFields: FC<HttpOptionsFieldsProps> = ({ serviceIndex, exposeIndex }) => {
  ...
  const toggleCustomOptions = useCallback(
    (checked: boolean) => {
      if (checked && !getValues(`${basePath}.httpOptions`)) {
