...
describe(ConfigureDeploymentForm.name, () => {
  ...
  function setup(input: { initialSdl: string | undefined; Panes?: typeof SdlProbePanes; draftId?: string; save?: ReturnType<typeof vi.fn> }) {
    const ConfigureDeploymentPanes = vi.fn(input.Panes ?? (() => <div data-testid="panes-mock" />));
    const enqueueSnackbar = vi.fn();
    const Snackbar = vi.fn(() => null);
    const save = input.save ?? vi.fn();
    const useConfigureDraft = vi.fn(() => mock<ReturnType<typeof DEPENDENCIES.useConfigureDraft>>({ read: vi.fn(() => undefined), save, clear: vi.fn() }));
