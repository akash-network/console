...
export class NotificationRouterService {
  ...
  async send(
    notificationCommand: NotificationCommandDto,
  ): Promise<Result<void, RichError>> {
    const contactPoint = await this.contactPointRepository.findById(
      notificationCommand.contactPointId,
    );
    ...
    if (contactPoint.type === 'email') {
      ...
      return Err(
        new RichError(
          'ContactPoint type not implemented',
          'UNSUPPORTED_CONTACT_POINT_TYPE',
          { contactPoint },
        ),
      );
    }
