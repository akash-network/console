...
export type ContactPointOutputResponse = z.infer<
  typeof contactPointOutputResponseSchema
>;
...
export class ContactPointController {
  ...
  async createContactPoint(
    @Body() { data }: { data: z.infer<typeof contactPointCreateInputSchema> },
  ): Promise<Result<ContactPointOutputResponse, unknown>> {
    return Ok({
  ...
  private toResponse(
    contactPoint: ContactPointOutput | undefined,
