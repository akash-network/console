...
> {
  ...
  throwUnlessCanExecute(payload: Record<string, any>) {
    const params = [
      this.action,
      subject(this.subjectType as string, payload),
    ] as unknown as Parameters<A['can']>;
  ...
  private toDrizzleWhereClause() {
    const params = [this.action, this.subjectType] as unknown as Parameters<
      A['can']
    >;
    ForbiddenError.from(this.ability).throwUnlessCan(...params);
    ...
    const conditions: (FieldCondition | CompoundCondition<FieldCondition>)[] =
      $and;
  ...
  private buildCondition(
    condition:
      | CompoundCondition<FieldCondition | CompoundCondition<FieldCondition>>
      | FieldCondition,
  ): SQL | undefined {
    ...
    if (condition instanceof CompoundCondition) {
      switch (condition.operator.toLowerCase()) {
        case 'and':
          return condition.value.length === 1
            ? this.buildCondition(condition.value[0])
            : and(
                ...condition.value.map((value) => this.buildCondition(value)),
              );
        case 'or':
          return condition.value.length === 1
            ? this.buildCondition(condition.value[0])
            : or(...condition.value.map((value) => this.buildCondition(value)));
      }
    }
