...
import { generateMessageTypeFilters, generateMockBlockData, generateMockMessages, generateSimpleMockBlock } from "@test/seeders";
...
describe(BlockMessageService.name, () => {
  ...
  describe("getMessages", () => {
    it("should fetch a block and parse its messages", async () => {
      const { service, blockchainClientService, blockMessageParserService } = await setup();
      ...
      blockMessageParserService.parseBlockMessages.mockReturnValue(mockBlockData);
  ...
  async function setup(): Promise<{
    ...
    const module = await Test.createTestingModule({
      providers: [BlockMessageService, MockProvider(BlockchainClientService), MockProvider(BlockMessageParserService)]
    }).compile();

    return {
      module,
      service: module.get<BlockMessageService>(BlockMessageService),
      blockchainClientService: module.get<MockProxy<BlockchainClientService>>(BlockchainClientService),
      blockMessageParserService: module.get<MockProxy<BlockMessageParserService>>(BlockMessageParserService)
    };
  }
});
