...
type Props = {
  ...
  providers: Array<MergedProvider> | Array<ApiProviderList>;
};
...
export const ProviderMap: React.FunctionComponent<Props> = ({ providers, initialZoom = minZoom, initialCoordinates = [0, 0] }) => {
  ...
  const activeProviders = providers.filter(x => (x as MergedProvider).isActive || (x as ApiProviderList).isOnline);
