...
- **indexer** ([`apps/indexer`](../apps/indexer)) - downloads Akash chain blocks and indexes them into the chain DB. Plain Node + Hono (status endpoint on port 3079) driven by a custom scheduler. See [Indexer pipeline](#indexer-pipeline) below.
