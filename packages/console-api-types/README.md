...
- `src/chain-indexer/` — the same pair generated from `apps/chain-indexer/swagger/openapi.json` (`npm run swagger:gen -w apps/chain-indexer`, then `npm run generate:chain-indexer` here), exported as `@akashnetwork/console-api-types/chain-indexer`.
